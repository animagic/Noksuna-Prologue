...
float4 frag(v2f i) : COLOR
{
	...
	for (int s = 1; s < STEP_COUNT && RayLength>0; s += 1, t += dt, RayLength -= dt)
	{
		...
		VolumeSpaceCoordsWorldSpace = mul(unity_ObjectToWorld, VolumeSpaceCoords) + _VolumePosition;
...
//#ifdef VOLUMETRIC_SHADOWS
		UNITY_BRANCH
			if (VOLUMETRIC_SHADOWS==1) {
				half2 shadowUVs = (mul(_ShadowCameraProjection, VolumeSpaceCoordsWorldSpace - _ShadowCameraPosition).xy + _ShadowCameraSize) / (_ShadowCameraSize*2.0f);
