...
public class FogVolumeDirectionalLight : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        if (!_GO_Image)
        {
            ...
            _CanvasImage.rectTransform.position = new Vector3(110, 320, 0);
    ...
    void Update()
    {
        ...
        if (_TargetFogVolumes != null && _AtLeastOneFogVolumeInArray())
        {
            ...
            //for (int FVindex = 0; FVindex < _TargetFogVolume.Length; FVindex++)
            //{
            //    if (_TargetFogVolume[FVindex] != null && _TargetFogVolume[FVindex].enabled)
            //    {
            //        FogVolumeMaterial = _TargetFogVolume[FVindex].FogMaterial;

            //        //  else
            //        FogVolumeMaterial.SetInt("_VolumetricShadowsEnabled", depthRT ? 1 : 0);
            //    }
            //}
            //  FogVolumeUtilities.Rendering.EnsureKeyword(FogVolumeMaterial, "VOLUMETRIC_SHADOWS", depthRT ? true : false);
            //FogVolumeMaterial.EnableKeyword("VOLUMETRIC_SHADOWS");
            //print(FogVolumeMaterial.IsKeywordEnabled("VOLUMETRIC_SHADOWS"));
